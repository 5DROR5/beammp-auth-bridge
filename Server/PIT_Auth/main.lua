...
local DescUpdater = dofile(ROOT .. "/modules/DescUpdater.lua")

-- =============================================================================
-- LOGGING
-- =============================================================================

local function log(msg) Util.LogInfo(PLUGIN .. " " .. tostring(msg)) end
...
local function saveStore()
    ...
    local ok, err = FS.Rename(tmp, DB_PATH)
    if not ok then
...
local function trigger(pid, event, data)
    ...
    MP.TriggerClientEvent(pid, event, data or "")
end

local function enc(t) return Util.JsonEncode(t) or "{}" end
...
function PIT_AUTH_BroadcastNames()
    ...
    for pid, _ in pairs(MP.GetPlayers()) do
        if MP.IsPlayerConnected(pid) then
            MP.TriggerClientEvent(pid, "PIT_AUTH_NamesList", payload)
        end
    end
end
...
local function updatePrefix(pid)
    ...
    for id, _ in pairs(MP.GetPlayers()) do
        if MP.IsPlayerConnected(id) then
            MP.TriggerClientEvent(id, "PIT_AUTH_PrefixUpdate", payload)
        end
    end
end
...
function PIT_AUTH_OnJoin(pid)
    log("Player joined: pid=" .. pid .. " guest=" .. tostring(MP.IsPlayerGuest(pid)))
    if MP.IsPlayerGuest(pid) then
        pending[pid]          = true
...
function PIT_AUTH_OnAuth(pid, raw)
    ...
    local data = Util.JsonDecode(raw)
...
function PIT_AUTH_DescTick() DescUpdater.updateDesc() end
...
function PIT_AUTH_OnInit()
    ...
    loadStore()
    loadTranslations()
    DescUpdater.init({ MP = MP, auth_names = auth_names, log = log })
    log("=== PIT Auth ready ===")
end
